...
# Paths resolved relative to this folder (works regardless of current working directory)
BASE_DIR = Path(__file__).resolve().parent
LEVELS_DIR = BASE_DIR / 'levels'
HIGHSCORE_PATH = BASE_DIR / 'highscore.txt'
...
ENABLE_CHASER = False
ENABLE_POWER = False
ENABLE_EAT_GHOST = False
ENABLE_LEVELS = False
ENABLE_HIGHSCORE = False

# Speed controls (tiles per move step; lower tick interval = faster)
# These values are "move every N frames" at 60 FPS.
PACMAN_MOVE_EVERY_TICKS = 4
RANDOM_GHOST_MOVE_EVERY_TICKS = 6
CHASER_GHOST_MOVE_EVERY_TICKS = 6
...
class Pacman(Actor):
    def __init__(self, cell: Cell) -> None:
        super().__init__(cell, dir=Direction.LEFT, nextDir=Direction.LEFT, moveEveryTicks=PACMAN_MOVE_EVERY_TICKS)
        self.powerTicks = 0
        self.ghostCombo = 0
...
class RandomGhost(Ghost):
    def __init__(self, cell: Cell, home: Cell) -> None:
        super().__init__(cell, home, moveEveryTicks=RANDOM_GHOST_MOVE_EVERY_TICKS)
...
class ChaserGhost(Ghost):
    def __init__(self, cell: Cell, home: Cell) -> None:
        super().__init__(cell, home, moveEveryTicks=CHASER_GHOST_MOVE_EVERY_TICKS)
...
class Game:
    def __init__(self) -> None:
        ...
        if ENABLE_HIGHSCORE:
            self.score.load_high_score(str(HIGHSCORE_PATH))
    ...
    def _spawn_default_ghosts(self) -> None:
        ...
        for i, spawn in enumerate(spawns):
            if ENABLE_CHASER and i == 0:
                self.ghosts.append(ChaserGhost(spawn, home=spawn))
            else:
                self.ghosts.append(RandomGhost(spawn, home=spawn))
    ...
    def load_level(self, n: int) -> None:
        ...
        if ENABLE_LOADER:
            ...
            path = str(LEVELS_DIR / f'level{n}.txt')
            try:
                self.maze = load_level_from_ascii(path)
            except FileNotFoundError:
                self.level = 1
                self.maze = load_level_from_ascii(str(LEVELS_DIR / 'level1.txt'))
        else:
            self.maze = self._make_fallback_maze()
                ...
                if g.cell == self.pacman.cell:
                    ...
                    if edible:
                        ...
                        if self.score.get_lives() <= 0:
                            ...
                            if ENABLE_HIGHSCORE:
                                self.score.save_high_score(str(HIGHSCORE_PATH))
                        else:
