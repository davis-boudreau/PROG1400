...
from pacman_skeleton import Maze, TileType, Cell, Item, Pellet, PowerPellet, Fruit
...
def load_level_from_ascii(
    ...
    *,
    ...
    """
    Load an ASCII level file into a Maze.
    ...
    Symbols:
      ...
      . pellet
      o power pellet
      P pacman spawn (floor)
      G ghost spawn (floor)
      T tunnel (walkable + warp)
      F fruit (floor)

    Returns:
      Maze with tiles, items, pacmanSpawn, ghostSpawns populated.
    """
    ...
    # Make rectangular by padding shorter lines
...
def _read_lines_keep_spaces(path: str) -> List[str]:
    ...
    # Remove trailing empty lines (common at end of file)
